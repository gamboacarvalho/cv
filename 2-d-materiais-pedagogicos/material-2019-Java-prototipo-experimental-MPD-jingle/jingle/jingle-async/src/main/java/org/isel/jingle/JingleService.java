...
import org.isel.jingle.util.ObservablesUtil;
...
public class JingleService {
    ...
    public JingleService() {
        this(new LastFmWebApi(new BaseRequestAsync()));
    }

    public Observable<Artist> searchArtist(String name) {
        return searchApiThenMapTo(page -> api.searchArtist(name, page), this::mapToArtist);
    }


    public Observable<TopTrack> getTopTracks(String country) {
        return searchApiThenMapTo(page -> api.getTopTracks(country, page), dto -> mapToTopTrack(dto, country));
    }

    public Observable<Track> getTracksRank(String artistMbid, String country) {
        Observable<Track> artistTracks = getArtistTracks(artistMbid);
        Observable<TopTrack> topTracks = getTopTracks(country).take(MAX_TRACK_RANKS_FOR_COUNTRY);

        TopTrack defaultTopTrack = new TopTrack("", "", country, 0, 0);

        return ObservablesUtil
                .merge(artistTracks, topTracks, this::compareTrackNames, this::addCountryRankToTrack, defaultTopTrack);
    ...
    public Observable<Track> getAlbumTracks(String albumMbid) {
        ...
        return searchApiThenMapTo(page -> api.getAlbumInfo(albumMbid), this::mapToTrack);
    }

    public Observable<Album> getAlbums(String artistMbid) {
        return searchApiThenMapTo(page -> api.getAlbums(artistMbid, page), this::mapToAlbum);
    }


    public Observable<Track> getArtistTracks(String artistMbid) {
        return getAlbums(artistMbid).concatMap(Album::getTracks);
    }


    @SuppressWarnings("unchecked")
    private <T, R> Observable<R> searchApiThenMapTo(Function<Integer, CompletableFuture<T[]>> searchFunction, Function<? super T, R> modelObjectMapper) {
        return Observable
                .range(STARTING_PAGE, Integer.MAX_VALUE - 1)
                .map(searchFunction::apply)
                .concatMap(Observable::fromFuture)
                .filter(Objects::nonNull)
                .takeWhile(trackDtos -> trackDtos.length != 0)
                .concatMap(Observable::fromArray)
                .map(modelObjectMapper::apply);
    }
