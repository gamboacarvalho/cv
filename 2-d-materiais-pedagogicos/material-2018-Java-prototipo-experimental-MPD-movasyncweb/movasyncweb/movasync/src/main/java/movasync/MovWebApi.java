...
public class MovWebApi {
    ...
    public CompletableFuture<SearchDto> search(String title, int page) {
        ...
        return req
                .getBody(path)
                .thenApply(body -> Stream.of(body).reduce("", (prev, curr) -> prev + curr))
    ...
    public CompletableFuture<MovieDto> getMovie(int id) {
        ...
        return req
                .getBody(path)
                .thenApply(body -> Stream.of(body).reduce("", (prev, curr) -> prev + curr))
                .thenApply(json -> gson.fromJson( json , MovieDto.class));
    }

    /**
     * E.g. https://api.themoviedb.org/3/movie/860/credits?api_key=3206de5c5e8ebe8b47eca707e469e394
     */
    public CompletableFuture<CreditsDto> getMovieCredits(int movieId) {
        String path = MOVIE_DB_HOST +
        ...
        return req
                .getBody(path)
                .thenApply(body -> Stream.of(body).reduce("", (prev, curr) -> prev + curr))
                .thenApply(json -> gson.fromJson( json , CreditsDto.class));
    }
    ...
    public CompletableFuture<PersonDto> getPerson(int personId) {
        ...
        return req
                .getBody(path)
                .thenApply(body -> Stream.of(body).reduce("", (prev, curr) -> prev + curr))
                .thenApply(json -> gson.fromJson( json , PersonDto.class));
    }
    ...
    public CompletableFuture<SearchItemDto[]> getPersonCreditsCast(int personId) {
        ...
        return req
                .getBody(path)
                .thenApply(body -> Stream.of(body).reduce("", (prev, curr) -> prev + curr))
                .thenApply(json -> gson.fromJson( json , SearchPersonCreditsDto.class).getCast());
    }
}