...
namespace Jsonzai {
    ...
    public class JsonParser {
        ...
        private static Dictionary<Type, KlassInfo> klassInfoDic = new Dictionary<Type, KlassInfo>();
        

        private static KlassInfo GetKlassInfo(Type key) {
            if (!klassInfoDic.TryGetValue(key, out KlassInfo klass))
            {
                klass = new KlassInfo(key);
        ...
        private static object FillObject(BaseJsonTokens tokens, object target) {
            ...
            KlassInfo klassInfo = GetKlassInfo(type);
            BaseSetter setter;
            string key;
        ...
        public static void ClearConfiguration() {
            foreach(KlassInfo klass in klassInfoDic.Values) {
                klass.setters.Clear();
